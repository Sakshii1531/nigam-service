...
async function createServiceProvider(request, specs) {
  const phone = uniquePhone();
  const createRes = await request.post('/api/v1/_dev/test-serviceProvider', { data: { phone, password: 'password123', specs } });
...
/** category/service/service provider scoped to one random key — same isolation
 * reasoning as booking.spec.js/service provider.spec.js. */
...
test.describe('notifications — real domain events, not just CRUD', () => {
  test('a booking fires booking.created and serviceProvider.assigned for the customer', async ({ request }) => {
...
test.describe('chat — conversation auto-created on job acceptance', () => {
  test('accepting a job creates a conversation visible to both the customer and the serviceProvider, with masked phone numbers', async ({ request }) => {
    const { categoryKey, provider, customer } = await setupBookingFixture(request);
    ...
    expect(customerList[0].serviceProvider.phone).not.toBe(provider.phone);

    const serviceProviderConvos = await request.get('/api/v1/chat/conversations', { headers: { Authorization: `Bearer ${provider.token}` } });
    const serviceProviderList = (await serviceProviderConvos.json()).data;
    expect(serviceProviderList).toHaveLength(1);
    expect(serviceProviderList[0].id).toBe(customerList[0].id);
  });
