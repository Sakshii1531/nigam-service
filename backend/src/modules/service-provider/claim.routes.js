...
// Manual FOC claims a service provider raises directly (RaisePartRequest.jsx) —
// distinct from the auto-created claims job.service.js raises when spare parts
// are used on a warranty/AMC-covered job.
export const claimRouter = Router();
claimRouter.use(requireAuth, requireRole(ROLES.SERVICE_PROVIDER), attachServiceProvider);

claimRouter.post('/', validate(raiseClaimSchema), async (req, res, next) => {
  try {
    created(res, await claimService.raiseServiceProviderClaim(req.serviceProvider.id, req.body));
  } catch (err) {
    next(err);
  }
});
...
claimRouter.get('/', validate(listClaimsQuerySchema, 'query'), async (req, res, next) => {
  try {
    const { items, meta } = await claimService.listServiceProviderClaims(req.serviceProvider.id, req.query);
    ok(res, items, meta);
  } catch (err) {
    next(err);
  }
...
claimRouter.get('/:id', validate(claimIdParamSchema, 'params'), async (req, res, next) => {
  try {
    ok(res, await claimService.getServiceProviderClaim(req.serviceProvider.id, req.params.id));
  } catch (err) {
    next(err);
  }
});
