...
function canView(serviceRequest, reqUser, serviceProviderId) {
...
serviceRequestRouter.get('/:id', validate(idParamSchema, 'params'), async (req, res, next) => {
  try {
    ...
    if (!canView(serviceRequest, req.user, serviceProviderId)) throw new ApiError(403, 'Not authorized to view this request');
...
serviceRequestRouter.get(
  '/:id/serviceProvider-suggestions',
