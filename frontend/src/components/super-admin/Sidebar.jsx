...
const Sidebar = () => {
  ...
  const [openSections, setOpenSections] = useState({
    customerApp: location.pathname.includes(
      "/super-admin/customer-app-customization",
    ),
  ...
  return (
    <div
      className={`w-64 bg-[#F4F7FE] h-screen border-r border-[#E2E8F0] flex flex-col fixed left-0 top-0 z-30 transition-transform duration-300 ease-in-out ${
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center">
                <span className="text-base font-extrabold text-[#0D47A1] tracking-tight">
                  NIGAM
                </span>
                <span className="text-base font-extrabold text-[#FFB300] tracking-tight ml-1">
                  CARE
                </span>
              </div>
              <p className="text-[10px] text-[#64748B] font-bold tracking-wider leading-none mt-0.5">
                {isAsm ? "AREA SERVICE MANAGER" : "SUPER ADMIN"}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="absolute bottom-16 left-4 right-4 bg-white border border-[#E2E8F0] rounded-xl shadow-lg py-1.5 z-30 animate-in fade-in slide-in-from-bottom-2 duration-150">
            <button
              onClick={() => {
                ...
                navigate(
                  isAsm ? "/super-admin/profile" : "/super-admin/settings",
                );
              }}
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="truncate min-w-0 leading-tight">
              ...
              <p className="text-[10px] text-[#64748B] truncate">
                {user?.email || user?.phone || "superadmin@ncc.com"}
              </p>
            </div>
          </div>
          <ChevronDown
            size={14}
            className={`text-[#64748B] shrink-0 transition-transform duration-200 ${isProfileMenuOpen ? "rotate-180" : ""}`}
          />
        </div>
      </div>
