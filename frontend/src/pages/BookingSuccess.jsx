...
const BookingSuccess = () => {
  ...
  const handleSearchNow = useCallback(() => {
    navigate(`/searching-partner${location.search}`);
  }, [navigate, location.search]);

  // 5-second auto redirect to searching-partner screen
  useEffect(() => {
    if (countdown <= 0) {
      handleSearchNow();
      return;
  ...
  const totalPrice =
    totalPriceParam || (charged?.total != null ? String(charged.total) : "0");
  const advanceAmt =
    advanceAmtParam ||
    (charged?.advance != null ? String(charged.advance) : "0");

  // Time slot display
  const timeSlotDisplay = isInstant
    ? "⚡ Right Now (Instant ASAP Service)"
    : {
        Morning: "8 AM – 11 AM",
      ...
      <div className="flex-1 max-w-screen-2xl mx-auto w-full px-4 md:px-8 py-6 md:py-8 overflow-y-auto">
        ...
        <div className="bg-linear-to-b from-[#E8F0FF] to-[#F0F4FF] pt-8 md:pt-12 pb-6 flex flex-col items-center gap-3 px-4 rounded-3xl md:border md:border-blue-100/50 mb-6 shadow-xs">
          ...
          <div className="relative">
            {/* Sparkles */}
            <span className="absolute -top-3 -left-4 text-yellow-400 text-lg animate-pulse">
              ✦
            </span>
            <span className="absolute -top-2 right-0 text-blue-300 text-sm animate-pulse delay-75">
              ✦
            </span>
            <span className="absolute bottom-0 -right-5 text-yellow-300 text-sm animate-pulse delay-150">
              ✦
            </span>
          ...
          <div className="text-center">
            <h1 className="text-[22px] md:text-3xl font-extrabold text-slate-900 leading-tight">
              Booking Confirmed!
            </h1>
        ...
        <div className="bg-linear-to-r from-blue-600 via-indigo-600 to-blue-700 text-white rounded-2xl md:rounded-3xl p-4 md:p-5 shadow-md shadow-blue-500/20 mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs md:text-sm font-black">
                  Searching Service Partner
                </span>
                <span className="text-[10px] font-extrabold bg-white/25 px-2.5 py-0.5 rounded-full animate-pulse">
                  Redirecting in {countdown}s
                </span>
              </div>
              <p className="text-[11px] md:text-xs text-blue-100 font-medium mt-0.5">
                We are searching and assigning the best service partner for your
                service.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
            <div className="flex-1 sm:w-28 h-2 bg-white/20 rounded-full overflow-hidden">
              <div
            ...
            <button
              ...
              className="shrink-0 bg-white text-brand-blue font-extrabold text-xs px-4 py-2 rounded-xl shadow-xs hover:bg-blue-50 active:scale-95 transition-all flex items-center gap-1.5 cursor-pointer">
        ...
        <div className="flex flex-col md:grid md:grid-cols-12 md:gap-8 items-start">
          ...
          <div className="w-full md:col-span-7 lg:col-span-8 flex flex-col gap-4 text-left">
            ...
            <div className="bg-white rounded-2xl md:rounded-3xl px-5 py-3.5 shadow-xs border border-slate-100 flex items-center justify-between">
              ...
              <span className="text-[12px] md:text-sm font-extrabold text-slate-900 tracking-wider">
                {bookingId || "—"}
              </span>
            </div>
                  ...
                  <span className="text-[11px] md:text-xs font-extrabold">
                    {isAccepted && serviceProvider
                      ? instantStatus === "EN_ROUTE"
                        ? "🚗 On The Way"
                        : "✅ Service Provider Booked"
                      : "⏳ Looking for Nearby Tech..."}
                  </span>
                </div>
                <div className="flex items-center gap-3.5 mt-3">
                  ...
                  <div className="flex-1">
                    <p className="text-xs md:text-sm font-black leading-tight">
                      {isAccepted && serviceProvider
                        ? `${serviceProvider.name || "Service Provider"} is on the way!`
                        : "Searching nearest certified serviceProvider in your territory"}
                    </p>
                    <p className="text-[11px] text-white/90 font-medium mt-0.5">
                      {isAccepted && serviceProvider
                        ? "Estimated arrival: within 20-35 minutes"
                        : "Dispatching candidate by proximity • Nearest to farthest"}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl md:rounded-3xl shadow-md border-2 border-emerald-500/30 p-5 md:p-6 relative overflow-hidden transition-all duration-500">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs md:text-sm font-black text-emerald-700 uppercase tracking-wide">
                      {instantStatus === "EN_ROUTE"
                        ...
                        : "Service Provider Booked for Your Service"}
                    </span>
                  </div>
                ...
                <div className="mt-4 pt-3.5 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                  <span className="text-slate-500 font-medium flex items-center gap-1.5">
                    ...
                    This serviceProvider is booked for your service and on the
              ...
              <div
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 font-medium mt-0.5">
                        Redirecting to live partner radar in {countdown}s, or
                        tap here to view now.
                      </p>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="shrink-0 bg-brand-blue text-white px-4 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 shadow-xs hover:bg-[#1565C0] active:scale-95 transition-all cursor-pointer">
                    <span>View Search Radar</span>
                    <ArrowRight className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-3 md:hidden mt-2">
              <button
                onClick={handleSearchNow}
                className="w-full bg-brand-blue text-white font-extrabold py-4 rounded-2xl text-[15px] shadow-md shadow-brand-blue/25 hover:bg-[#1565C0] active:scale-[0.98] transition-all flex items-center justify-center gap-2 cursor-pointer">
                <Radio className="w-4 h-4 animate-pulse" />
                <span>Go to Search Screen</span>
                <ArrowRight className="w-4 h-4" />
              </button>

              <button
                onClick={() => navigate("/my-bookings")}
                className="w-full bg-white text-slate-700 font-extrabold py-3.5 rounded-2xl text-[14px] border border-slate-200 hover:bg-slate-50 active:scale-[0.98] transition-all flex items-center justify-center gap-2 cursor-pointer">
                View My Bookings
              </button>

              <button
                onClick={() => navigate("/dashboard")}
                className="w-full text-center text-[13px] font-extrabold text-slate-600 hover:text-slate-900 transition-colors py-1 cursor-pointer">
                Go to Home
              </button>
            </div>
          ...
          <div className="w-full md:col-span-5 lg:col-span-4 hidden md:flex flex-col gap-6 sticky top-24 text-left">
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm flex flex-col gap-4">
              ...
              <button
                onClick={() => navigate("/my-bookings")}
                className="w-full bg-white text-slate-700 font-bold py-3.5 rounded-2xl text-xs border border-slate-200 hover:bg-slate-50 active:scale-[0.98] transition-all flex items-center justify-center gap-2 cursor-pointer">
                View My Bookings
              </button>

              <button
                onClick={() => navigate("/dashboard")}
                className="w-full text-center text-xs font-bold text-slate-600 hover:text-slate-900 transition-colors py-2 rounded-xl hover:bg-slate-50 cursor-pointer">
                Go to Home
              </button>
            </div>
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div
                    key={item.title}
                    className="flex flex-col gap-1 p-3 bg-slate-50 rounded-2xl border border-slate-100">
