...
const AreaNotServiceable = () => {
  ...
  const { currentLocation, selectCity, openLocationModal } =
    useLocationContext();
  ...
  // Extract user's registered or selected city
  const userAddress =
  ...
  useEffect(() => {
    async function load() {
      ...
      if (
        userCity &&
        userCity !== "Your Area" &&
        isCityServiceable(userCity, cities)
      ) {
    ...
    try {
      if (user && user.role === "customer") {
        ...
        try {
          ...
          console.warn(
            "Could not save address to server, continuing locally",
            apiErr,
          );
          updatedAddresses = [newAddress];
        }

        const defaultIdx = updatedAddresses.findIndex(
          (a) => a.isDefault || a.city === selectedCity.name,
        );
        if (defaultIdx > 0) {
      ...
      await selectCity(selectedCity.name, selectedCity.state || "");
      setShowChangeCityModal(false);
      navigate("/dashboard", { replace: true });
    } catch (err) {
      console.error("Failed to update city:", err);
      setUpdateError(
        err?.message || "Failed to update city. Please try again.",
      );
    } finally {
  ...
  const filteredActiveCities = activeCities.filter(
    (c) =>
      c.name.toLowerCase().includes(searchCity.trim().toLowerCase()) ||
      (c.state &&
        c.state.toLowerCase().includes(searchCity.trim().toLowerCase())),
  );
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#F4F7FC] via-[#EEF4FB] to-[#E3ECF9] flex flex-col items-center justify-between p-4 sm:p-6 relative overflow-hidden text-slate-800">
      ...
      <header className="w-full max-w-2xl flex items-center justify-between py-2 relative z-10">
        ...
        {user ? (
          <button
            onClick={logout}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:text-rose-600 bg-white/70 hover:bg-white border border-slate-200/80 rounded-full shadow-xs transition-all cursor-pointer">
            <LogOut size={13} />
            <span>Sign Out</span>
          </button>
        ) : (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-brand-blue hover:text-blue-700 bg-white/70 hover:bg-white border border-slate-200/80 rounded-full shadow-xs transition-all cursor-pointer">
            <span>Sign In</span>
          </button>
        )}
      </header>
      ...
      <main className="w-full max-w-lg my-auto py-6 relative z-10">
        <div className="bg-white/90 backdrop-blur-2xl border border-white/60 shadow-[0_20px_60px_rgba(13,71,161,0.08)] rounded-[32px] p-6 sm:p-8 text-center flex flex-col items-center relative overflow-hidden">
          ...
          <p className="text-slate-600 text-xs sm:text-sm max-w-md mb-6 leading-relaxed">
            ...
            <strong className="text-slate-900 font-bold">{userCity}</strong>{" "}
            right now. We are expanding rapidly across India and will be
            launching in your neighborhood soon!
          </p>
          ...
          <div className="w-full space-y-3">
            ...
            {!notified ? (
              ...
              <motion.div
                ...
                <span>
                  You&apos;re on the priority list! We will notify you upon
                  launch.
                </span>
              </motion.div>
            )}
            ...
            <button
              ...
              className="w-full bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-bold py-2.5 px-4 rounded-2xl transition-all shadow-xs flex items-center justify-center gap-2 text-xs cursor-pointer active:scale-98">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
            <motion.div
              ...
              <p className="text-xs text-slate-500 mt-2 mb-3">
                Select one of our active cities below to unlock all services and
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
            <motion.div
              ...
              <p className="text-xs text-slate-500 mt-2 mb-4">
                Nigam Care doorstep serviceProviders and verified experts are
                currently operating in these regions:
              </p>
