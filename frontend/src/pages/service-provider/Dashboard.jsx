...
import { 
  Bell, Briefcase, ClipboardList, Calendar, Wrench, User, MapPin, ChevronRight, ChevronLeft,
  Clock, Shield, Star, GraduationCap, MessageSquare, Megaphone, CheckCircle, RotateCw, LogOut, Zap, Wallet,
...
const Dashboard = () => {
  ...
  React.useEffect(() => {
    const handleIncomingEvent = (e) => {
      ...
      const ids = [payload?.serviceRequestId, payload?.bookingId].filter(Boolean).map(String);
      if (ids.length) setDispatchedIds((prev) => [...new Set([...prev, ...ids])]);
    };
  ...
  React.useEffect(() => {
    ...
    const handleCancelledEvent = (e) => {
      ...
      const targetId = payload.bookingId || payload.serviceRequestId || payload.id;
      if (instantAlertJob && (instantAlertJob.id === targetId || instantAlertJob.serviceRequestId === targetId || instantAlertJob.bookingId === targetId)) {
        setInstantAlertJob(null);
      }
      if (rescheduleAlertJob && (rescheduleAlertJob.jobId === targetId || rescheduleAlertJob.bookingId === targetId || rescheduleAlertJob.serviceRequestId === targetId)) {
        setRescheduleAlertJob(null);
      }
    };

    window.addEventListener('service-provider:job_rescheduled', handleRescheduleEvent);
    window.addEventListener('service-provider:job_cancelled', handleCancelledEvent);
    return () => {
      window.removeEventListener('service-provider:job_rescheduled', handleRescheduleEvent);
      window.removeEventListener('service-provider:job_cancelled', handleCancelledEvent);
    };
  }, [instantAlertJob, rescheduleAlertJob]);

  // Check for any active job with providerRescheduleStatus === 'PENDING'
  React.useEffect(() => {
    const pendingReschedule = jobs.find(j => j.providerRescheduleStatus === 'PENDING');
    if (pendingReschedule && !rescheduleAlertJob) {
      setRescheduleAlertJob({
        ...
        bookingId: pendingReschedule.booking?.humanId || pendingReschedule.booking?.id || pendingReschedule.id,
  ...
  React.useEffect(() => {
    ...
    if (instantAlertJob) {
      ...
      const latest = openOffers.find((j) => j.serviceRequestId === instantAlertJob.serviceRequestId);
  ...
  if (instantAlertJob?.assignedToMe && alertOpenedAt) {
    ...
    const base = Number.isFinite(assignedMs) && elapsedAtOpen >= 0 && elapsedAtOpen < OFFER_WINDOW_MS ? assignedMs : alertOpenedAt;
    alertDeadline = base + OFFER_WINDOW_MS;
  }
  const countdown = alertDeadline ? Math.max(0, Math.ceil((alertDeadline - alertNow) / 1000)) : null;
  ...
  const JOB_KIND_STYLE = {
    paid: { label: 'Paid service', border: 'border-l-[#2E7D32]', chip: 'bg-[#E8F5E9] text-[#2E7D32]', icon: ClipboardList },
    warranty: { label: 'Warranty', border: 'border-l-[#1E6BDB]', chip: 'bg-[#E3F2FD] text-[#1565C0]', icon: Shield },
    ew: { label: 'Ext. warranty', border: 'border-l-[#7C4DFF]', chip: 'bg-[#F3E5F5] text-[#6A1B9A]', icon: Shield },
    amc: { label: 'AMC visit', border: 'border-l-[#FFA000]', chip: 'bg-[#FFF3E0] text-[#E65100]', icon: Calendar },
  };

  const isPriorityJob = (job) => job.isPriority || job.priority === 'High' || job.priority === 'Critical';
  ...
  const JOB_FILTERS = [
    ...
    { id: 'Warranty', label: 'Warranty', test: (j) => jobKind(j) === 'warranty' || jobKind(j) === 'ew' },
  ...
  const activeFilter = JOB_FILTERS.find((f) => f.id === filterTab) || JOB_FILTERS[0];
  ...
  const revisitJobs = activeJobs.filter(job => (
  ...
  const initials = (user?.name || 'P')
    ...
    .join('');
  const completedToday = summary?.completedToday ?? earningsTally.completedToday ?? 0;
  ...
  const renderBrandLogo = (brandName) => {
    ...
    if (b.includes('lg')) return <span className="font-bold text-[#C60C30] text-xs tracking-tight">LG</span>;
    if (b.includes('samsung')) return <span className="font-black text-[#0A2C74] text-[11px] tracking-wider">SAMSUNG</span>;
    if (b.includes('voltas')) return <span className="font-black text-[#0D47A1] italic text-xs">VOLTAS</span>;
    ...
    <div className="service-provider-app-container min-h-screen bg-[#F4F7FC] flex flex-col pb-28 lg:pb-8 relative font-sans">
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              <div className="h-7 px-1.5 rounded-md bg-[#FFD400] flex items-center justify-center">
                <span className="text-[#052355] font-black text-xs tracking-tight">NCC</span>
              </div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 h-9 rounded-full pl-2.5 pr-3.5 text-xs font-semibold transition-colors disabled:opacity-60 cursor-pointer ${
                isOnline ? 'bg-green-500/20 text-green-200' : 'bg-white/10 text-slate-200'
              }`}
            >
              <span className={`w-2.5 h-2.5 rounded-full ${isOnline ? 'bg-green-400' : 'bg-slate-400'}`} />
              {!availabilityKnown
                ? 'Checking status…'
                : availabilityBusy
                  ? 'Updating…'
            ...
            {Boolean(summary?.rating != null && (summary?.reviewCount ?? 0) > 0) && (
              <span className="inline-flex items-center gap-1.5 h-9 rounded-full px-3 bg-white/10 text-xs font-semibold">
                ...
                {summary.rating.toFixed(1)} · {summary.reviewCount} review{summary.reviewCount === 1 ? '' : 's'}
              </span>
            )}
          </div>

          {dutyMessage && (
            <p className="text-xs text-blue-100 mt-2 leading-snug">{dutyMessage}</p>
          )}
        </div>
      )}
      ...
      <div className="flex-1 px-4 pt-4 pb-2 md:px-6 md:py-6 xl:px-8 flex flex-col gap-4 md:gap-5 max-w-screen-xl mx-auto w-full">
        ...
        {!showAllJobs && (
          <>
              ...
              {[
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-xl font-bold text-[#052355] leading-none tabular-nums">{stat.value}</p>
            ...
            >
              ...
              <div className="flex-1 grid grid-cols-2 gap-3 min-w-0">
                <div className="min-w-0">
                  ...
                  <p className="text-base font-bold text-[#052355] tabular-nums">{inr(earningsTally.today)}</p>
                </div>
                <div className="min-w-0">
                  <p className="text-xs text-slate-500">Wallet balance</p>
                  <p className="text-base font-bold text-[#052355] tabular-nums">{inr(earningsTally.available)}</p>
                </div>
              </div>
            ...
            {priorityJobsCount > 0 && (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-[#B71C1C]">{priorityJobsCount} high-priority offer{priorityJobsCount === 1 ? '' : 's'}</p>
                    <p className="text-xs text-slate-600 mt-0.5">Time-sensitive jobs that need attention</p>
                  </div>
                </div>
              ...
              <section className="flex flex-col gap-2.5">
                <div className="flex items-baseline justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Waiting on spare parts or a follow-up visit</p>
                  </div>
                  <span className="text-xs font-bold text-[#0D47A1] bg-blue-50 px-2.5 py-1 rounded-full">{revisitJobs.length}</span>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                  {revisitJobs.map((job) => {
                    let stage = { label: 'Part dispatch pending', tone: 'bg-amber-50 text-amber-800', accent: 'border-l-amber-500', action: 'Open job' };
                    if (job.activeStep === 'revisit_ontheway') {
                      stage = { label: 'On the way', tone: 'bg-emerald-50 text-emerald-800', accent: 'border-l-emerald-600', action: 'Continue' };
                    } else if (job.activeStep === 'revisit_arrived') {
                      stage = { label: 'Arrived', tone: 'bg-purple-50 text-purple-700', accent: 'border-l-purple-600', action: 'Resume' };
                    } else if (job.activeStep === 'revisit_scheduled' || job.revisitScheduledDate) {
                      stage = { label: 'Scheduled', tone: 'bg-blue-50 text-[#0D47A1]', accent: 'border-l-[#0D47A1]', action: 'Start revisit' };
                    }
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2 w-full">
                          <div className="min-w-0">
                            <p className="text-xs text-slate-500 truncate">{job.category}</p>
                            <p className="text-sm font-bold text-[#052355] truncate">{job.product || 'Service follow-up'}</p>
                          </div>
                          <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full shrink-0 ${stage.tone}`}>
                            {stage.label}
                          </span>
                        </div>
                        <div className="flex flex-col gap-1 text-xs text-slate-600 w-full">
                          <span className="flex items-center gap-1.5"><User className="w-3.5 h-3.5 text-slate-400" />{job.customerName}</span>
                          {dateDisplay && (
                        ...
                        <span className="self-end text-xs font-bold text-[#0D47A1] flex items-center gap-0.5">
                          {stage.action} <ChevronRight className="w-3.5 h-3.5" />
                        </span>
                      </button>
                    );
                  })}
            ...
            <section className="flex flex-col gap-2.5">
              <div className="flex justify-between items-center">
                <h3 className="text-base font-bold text-[#052355]">New job offers</h3>
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-center gap-2 w-full">
                          <span className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-2 py-0.5 rounded-full ${kind.chip}`}>
                        ...
                        <div className="flex justify-between items-end gap-3 w-full">
                          <div className="min-w-0">
                            ...
                            {(job.scheduledDateLabel || job.scheduledTime || job.distance != null) && (
                              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 mt-1">
                                ...
                                {job.distance != null && (
                                  <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-slate-400" />{job.distance} km</span>
                                )}
                              </div>
                            )}
                          </div>
                          <div className="text-right shrink-0">
                            ...
                            <p className="text-base font-bold text-emerald-700 tabular-nums">{inr(job.estEarnings)}</p>
                          </div>
                        </div>
                      </button>
                    );
            ...
            <div className="grid grid-cols-3 gap-2.5 md:gap-3">
              {[
                { label: 'Academy', icon: GraduationCap, tone: 'bg-[#E3F2FD] text-[#1565C0]', path: '/service-provider/academy' },
                { label: 'Get help', icon: MessageSquare, tone: 'bg-[#E8F5E9] text-[#2E7D32]', path: '/service-provider/technical-support' },
                { label: 'Notices', icon: Megaphone, tone: 'bg-[#FFF3E0] text-[#E65100]', path: '/service-provider/announcements' },
              ].map((link) => (
                <button
                  key={link.label}
                ...
                >
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${link.tone}`}>
                    <link.icon className="w-5 h-5" />
                  </div>
                  <span className="text-xs md:text-sm font-semibold text-[#052355]">{link.label}</span>
                </button>
              ))}
            </div>
          </>
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar -mx-4 px-4 lg:mx-0 lg:px-0">
              {JOB_FILTERS.map((f) => {
                ...
                return (
                  ...
                  >
                    {f.label} <span className={filterTab === f.id ? 'text-blue-200' : 'text-slate-400'}>{count}</span>
                  </button>
                );
              })}
            </div>
            ...
            <div className="flex flex-col gap-3 lg:grid lg:grid-cols-2">
              {filteredJobs.length > 0 ? (
                  ...
                  return (
                    <div 
                      ...
                      className={`bg-white rounded-2xl p-4 flex flex-col gap-3 cursor-pointer border-l-4 ${kind.border} transition-all ${
                        isExpanded ? 'border border-[#0D47A1]/40 shadow-md' : 'border border-slate-200/80 hover:shadow-sm'
                      }`}
                    >
                      <div className="flex justify-between items-start gap-3">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${kind.chip}`}>
                              {kind.label}
                            </span>
                            <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${
                              job.isAvailableRequest ? 'bg-slate-100 text-slate-600' : 'bg-blue-50 text-[#0D47A1]'
                            }`}>
                              {job.isAvailableRequest ? 'New offer' : 'Active'}
                            </span>
                            {isPriorityJob(job) && (
                              <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-rose-50 text-rose-700">Priority</span>
                            )}
                          </div>

                          <p className="text-sm font-bold text-[#052355] mt-2 line-clamp-2 leading-snug">{job.product}</p>
                        ...
                        <div className="text-right shrink-0">
                          ...
                          <p className="text-base font-bold text-emerald-700 tabular-nums">{inr(job.estEarnings)}</p>
                          {job.price > 0 && (
                            <p className="text-xs text-slate-400 mt-0.5 tabular-nums">Bill {inr(job.price)}</p>
                          )}
                        </div>
                      </div>
                      ...
                      {job.isAvailableRequest ? (
                        isExpanded ? (
                          ...
                          <p className="text-xs text-slate-400">Tap to accept or decline</p>
                        )
                      ) : (
                        <button 
                ...
                <div className="lg:col-span-2 text-center py-10 bg-white rounded-2xl border border-dashed border-slate-300 px-4 text-slate-500">
                  ...
                  <p className="text-sm font-semibold text-slate-700">No jobs here</p>
                  <p className="text-xs mt-1">
                    {filterTab === 'All' ? 'New offers and active jobs will show up here.' : 'Try a different filter.'}
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="bg-white border border-slate-100 rounded-[32px] p-6 sm:p-7 max-w-sm w-full flex flex-col items-center text-center gap-4 shadow-2xl relative overflow-hidden">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium leading-relaxed px-1">
                You are currently offline. Would you like to turn your status <strong className="text-emerald-600 font-bold">Online</strong> to start receiving jobs, or <strong className="text-slate-700 font-bold">Keep Offline</strong>?
              </p>
            </div>
      ...
      {instantAlertJob && (() => {
        ...
        const when = job.isInstant
          ? 'As soon as possible'
          : [job.scheduledDateLabel, job.scheduledTime].filter(Boolean).join(', ') || 'Time to be confirmed';
              ...
              <div className="bg-gradient-to-br from-[#052355] to-[#0D47A1] text-white px-5 pt-5 pb-6">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <span className="inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wider text-blue-200">
                      ...
                      {job.assignedToMe ? 'New job for you' : 'New job nearby'}
                    </span>
                    <h3 id="dispatch-title" className="text-lg font-bold text-white mt-1.5 leading-snug line-clamp-2">
                      {job.product}
                    </h3>
                    <div className="flex flex-wrap gap-1.5 mt-2.5">
                      <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-white/15">{kind.label}</span>
                      {job.isInstant && (
                      ...
                      {isPriorityJob(job) && (
                        <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-rose-500">Priority</span>
                      )}
                    </div>
                  </div>

                  {countdown != null && (
                    <div className="relative w-14 h-14 shrink-0" aria-label={`${countdown} seconds left to respond`}>
                      ...
                      <span className={`absolute inset-0 flex items-center justify-center text-base font-bold tabular-nums ${urgent ? 'text-rose-300' : 'text-white'}`}>
                        {countdown}
                      </span>
                    </div>
                  )}
                ...
                <div className="mt-5 flex items-end justify-between gap-3">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-[#FFD400] tabular-nums leading-tight">{inr(job.estEarnings)}</p>
                  </div>
                  {job.price > 0 && (
                    <div className="text-right">
                      <p className="text-xs text-blue-200">Customer pays</p>
                      <p className="text-base font-semibold tabular-nums">{inr(job.price)}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="px-5 py-4 flex flex-col gap-3.5">
                {[
                  ...
                  <div key={row.label} className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-sm font-semibold text-slate-800 leading-snug">{row.value}</p>
                    </div>
                  </div>
                ))}
                ...
                {!job.assignedToMe && (
                  <p className="text-xs text-slate-600 bg-slate-50 rounded-xl px-3 py-2">
                    Offered to all partners nearby — the first to accept gets the job.
                  </p>
                )}
                {alertError && (
                  <p role="alert" className="text-sm text-rose-700 bg-rose-50 rounded-xl px-3 py-2">{alertError}</p>
                )}
              </div>
          ...
          <div className="bg-white border border-purple-100 rounded-[32px] max-w-md w-full flex flex-col shadow-2xl relative overflow-hidden">
            <div className="bg-gradient-to-r from-purple-700 via-indigo-700 to-purple-900 p-5 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-[10px] font-extrabold tracking-widest text-purple-200 uppercase block">RESCHEDULE REQUEST</span>
                  <h3 className="text-sm font-black text-white">Customer Changed Time</h3>
                </div>
              </div>
            </div>
            ...
            <div className="p-5 flex flex-col gap-4 text-left">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Customer: <strong className="text-slate-800">{rescheduleAlertJob.customerName || 'Customer'}</strong>
                </p>
              </div>

              <div className="bg-purple-50/70 border border-purple-200/80 rounded-2xl p-4 flex flex-col gap-2.5">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-bold text-slate-500 uppercase text-[10px]">New Requested Date</span>
                  <span className="font-extrabold text-purple-900">
                    {rescheduleAlertJob.scheduledDate ? new Date(rescheduleAlertJob.scheduledDate).toLocaleDateString('en-IN', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' }) : 'Rescheduled Date'}
                  </span>
                </div>
                <div className="flex items-center justify-between text-xs pt-1 border-t border-purple-200/50">
                  <span className="font-bold text-slate-500 uppercase text-[10px]">New Time Slot</span>
                  <span className="font-extrabold text-purple-900">
                    {typeof rescheduleAlertJob.timeSlot === 'object' ? (rescheduleAlertJob.timeSlot?.time || '10:00 AM - 01:00 PM') : (rescheduleAlertJob.timeSlot || '10:00 AM - 01:00 PM')}
                  </span>
                </div>
              ...
              <p className="text-xs text-slate-600 font-medium leading-relaxed">
                Are you available to attend this service at the rescheduled time? If you decline, this job will be reassigned to another available technician.
              </p>
              ...
              <div className="flex items-center gap-3 pt-2">
                <button
                  ...
                  onClick={async () => {
                    ...
                    const targetId = rescheduleAlertJob.jobId || rescheduleAlertJob.serviceRequestId || rescheduleAlertJob.bookingId;
                    await respondToReschedule(targetId, { action: 'reject', reason: 'Not available at this time' });
                    setRescheduleAlertJob(null);
                    setRespondingToReschedule(false);
                    setDutyMessage('Job declined and released to next available service provider.');
                  }}
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    setRescheduleAlertJob(null);
                    setRespondingToReschedule(false);
                    setDutyMessage('Rescheduled time accepted! Job schedule updated.');
                  }}
          ...
          <div className="bg-white border border-rose-100 rounded-[32px] max-w-md w-full flex flex-col shadow-2xl relative overflow-hidden">
            <div className="bg-gradient-to-r from-rose-600 to-red-700 p-5 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-[10px] font-extrabold tracking-widest text-rose-200 uppercase block">JOB CANCELLED</span>
                  <h3 className="text-sm font-black text-white">Customer Cancelled Booking</h3>
                </div>
              </div>
            </div>
            ...
            <div className="p-5 flex flex-col gap-4 text-left">
              <p className="text-xs text-slate-700 font-medium leading-relaxed">
                Booking <strong className="font-mono">{cancellationAlert.bookingId || cancellationAlert.humanId || 'Order'}</strong> has been cancelled by the customer.
                {cancellationAlert.reason && (
                  <span className="block mt-1 text-slate-500">Reason: "{cancellationAlert.reason}"</span>
                )}
              </p>
              <p className="text-[11px] text-slate-500">
                This booking has been removed from your active schedule and returned to available queue.
              </p>
