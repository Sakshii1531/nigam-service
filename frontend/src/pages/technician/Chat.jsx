...
const CANNED_REPLIES = [
  'Okay, thank you!',
  'Yes, the appliance is ready.',
  'Please come, I am waiting.',
  'Sure, no problem.',
];

/**
 * Technician-side masked chat with the customer. Mirrors the user Chat with
 * the technician as the sender. Simulated / in-memory.
 */
const TechChat = () => {
  const navigate = useNavigate();
  const { state } = useLocation();
  const customer = state?.customer || 'Amit Verma';
  const initial = customer.charAt(0).toUpperCase();

  const [message, setMessage] = useState('');
  const [messages, setMessages] = useState([
    { id: 1, from: 'customer', text: 'Hi, how long will you take?', time: '10:44 AM' },
    { id: 2, from: 'tech', text: 'On my way, reaching in 10 minutes.', time: '10:45 AM', status: 'read' },
  ]);
  ...
  const handleSend = (e) => {
    ...
    const newMsg = { id: Date.now(), from: 'tech', text: message.trim(), attachment, time: nowTime(), status: 'sent' };
    ...
    setTimeout(() => {
      ...
      replyIdx.current += 1;
      setMessages((prev) => [...prev, { id: Date.now() + 1, from: 'customer', text: reply, time: nowTime() }]);
    }, 2600);
  };

  const handleAttach = () => setAttachment({ name: `estimate_${Math.floor(Math.random() * 90 + 10)}.jpg` });
    ...
    <div className="min-h-screen bg-white flex flex-col">
      <div className="p-4 flex items-center border-b border-slate-200">
        ...
        <div className="flex items-center gap-3 ml-2 flex-1">
          ...
          <div>
            <h1 className="text-sm font-bold text-slate-900">{customer}</h1>
            <span className="text-xs font-semibold text-green-500">
              {customerTyping ? 'typing…' : sessionEnded ? 'session closed' : 'Customer · Online'}
            </span>
          </div>
        </div>
        <button
          onClick={() => navigate('/call', { state: { name: customer, role: 'Customer', backTo: '/technician/chat' } })}
        ...
        {!sessionEnded && (
          <button
            ...
            className="ml-2 p-2.5 rounded-full hover:bg-slate-100 transition-colors text-slate-400"
          >
            <X className="h-5 w-5" />
          </button>
        )}
      ...
      <div className="flex-1 p-4 flex flex-col gap-3 overflow-y-auto bg-slate-50">
        <div className="bg-white p-3 rounded-xl text-center text-xs text-slate-500 shadow-sm">
          🔒 Customer number is masked. Calls route through a secure line.
        </div>

        {messages.map((msg) =>
          msg.from === 'customer' ? (
            <div key={msg.id} className="flex flex-col items-start gap-1">
              <div className="bg-white p-3.5 rounded-t-2xl rounded-r-2xl max-w-[80%] shadow-sm text-sm text-slate-900">
                {msg.text}
              </div>
      ...
      {attachment && !sessionEnded && (
        <div className="px-4 pt-2 bg-white">
          <div className="inline-flex items-center gap-2 bg-slate-100 rounded-xl px-3 py-2">
            ...
            <span className="text-xs text-slate-900">{attachment.name}</span>
      ...
      {sessionEnded ? (
        <div className="p-4 border-t border-slate-200 bg-white">
          <div className="flex items-center justify-center gap-2 bg-slate-50 border border-slate-200 rounded-2xl py-3 text-xs text-slate-500">
        ...
        <form onSubmit={handleSend} className="p-4 border-t border-slate-200 bg-white flex items-center gap-3">
          <button type="button" onClick={handleAttach} className="p-3 text-slate-400 hover:text-[#0D47A1] transition-colors">
            <Paperclip className="h-5 w-5" />
          </button>
          <input
            type="text"
            placeholder="Message the customer..."
            className="flex-1 py-3 px-4 bg-slate-50 border border-slate-200 rounded-2xl focus:border-[#0D47A1] focus:ring-1 focus:ring-[#0D47A1] outline-none transition-all text-sm"
            value={message}
