...
const STATUS_BADGES = {
  ...
  'Parts Pending': { bg: 'bg-amber-50', text: 'text-amber-800', border: 'border-amber-200', dot: 'bg-amber-500' },
...
const BookingDetails = () => {
  ...
  const loadBooking = useCallback(async (isSilent = false) => {
    if (!id) return;
  ...
  const handleRescheduleBooking = async ({ scheduledDate, timeSlot, reason }) => {
    ...
    setActionLoading(true);
    try {
      await apiRequest(`/bookings/${booking.id || booking.humanId}/reschedule`, {
        method: 'POST',
  ...
  const timeStr = typeof booking.timeSlot === 'object'
  ...
  const isSpareReq = hasTl('Spare') || booking.instantStatus === 'PARTS_PENDING' || booking.status === 'Parts Pending';
    ...
    <div className="min-h-screen bg-[#F0F4FF] flex flex-col font-sans pb-28 sm:pb-12 text-slate-800">
      ...
      <main className="max-w-3xl mx-auto w-full px-4 py-5 flex flex-col gap-4">
        ...
        {!isCancelled && !isCompleted && (
          <div className="bg-gradient-to-r from-brand-navy to-[#0F3460] rounded-3xl p-4.5 text-white shadow-md flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
            ...
            >
              <span>{otpCode}</span>
              {copiedOtp ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-4 h-4 text-slate-400" />}
            </button>
          </div>
        )}
        ...
        <div className="bg-white rounded-3xl p-5 border border-slate-200/80 shadow-xs flex flex-col gap-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1">
            <div className="bg-slate-50 rounded-2xl p-3.5 border border-slate-100">
              <span className="text-[11px] text-slate-400 font-semibold block">Scheduled Date</span>
          ...
          {booking.rescheduledAt && (
            <p className="text-[11px] text-purple-700 bg-purple-50 p-2.5 rounded-xl border border-purple-200">
              Rescheduled on {new Date(booking.rescheduledAt).toLocaleString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}: {booking.rescheduleReason || 'Customer requested new slot'}
            </p>
          )}
        </div>
        ...
        <div className="bg-white rounded-3xl p-5 border border-slate-200/80 shadow-xs flex flex-col gap-3">
          ...
          {booking.serviceProvider ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1 mt-1 text-xs font-bold text-amber-600">
                    ...
                    <span>{booking.serviceProvider.rating || '4.9'} • 100+ Jobs Completed</span>
                  </div>
                </div>
              </div>

              {booking.serviceProvider.phone && !isCancelled && !isCompleted && (
                <a
            ...
            <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-4 flex items-center gap-3 text-xs text-slate-600">
              ...
              <p>
                <strong className="text-slate-800">Assigning nearest certified expert…</strong> We match technicians based on territory proximity and brand specialization.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-3xl p-5 border border-slate-200/80 shadow-xs flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-4 pl-2 border-l-2 border-blue-200 ml-3">
            ...
            <div className="relative pl-5">
              <div className="absolute -left-[19px] top-0 w-3.5 h-3.5 rounded-full bg-emerald-500 border-2 border-white shadow-xs" />
              <p className="text-xs font-black text-slate-900">Booking Placed & Confirmed</p>
              <p className="text-[11px] text-slate-400 mt-0.5">
                {booking.createdAt ? new Date(booking.createdAt).toLocaleString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : 'Confirmed'}
              </p>
            </div>

            {/* Assigned */}
            <div className="relative pl-5">
              <div className={`absolute -left-[19px] top-0 w-3.5 h-3.5 rounded-full border-2 border-white shadow-xs ${isAssigned ? 'bg-emerald-500' : 'bg-slate-300'}`} />
              <p className="text-xs font-black text-slate-900">
                {isAssigned ? 'Service Partner Allocated' : 'Allocating Nearest Partner'}
              </p>
              <p className="text-[11px] text-slate-400 mt-0.5">
                {isAssigned ? `${booking.serviceProvider.name} is scheduled for your visit` : 'Broadcasting request to active technicians'}
              </p>
            </div>
            ...
            {isSpareReq && (
              <div className="relative pl-5">
                <div className="absolute -left-[19px] top-0 w-3.5 h-3.5 rounded-full bg-amber-500 border-2 border-white shadow-xs" />
                <p className="text-xs font-black text-amber-900">Spare Parts In Transit</p>
                <p className="text-[11px] text-slate-500 mt-0.5">
                  Genuine parts requested from warehouse; technician will revisit to complete installation.
                </p>
              </div>
            )}
            ...
            {/* Final status */}
            <div className="relative pl-5">
              <div className={`absolute -left-[19px] top-0 w-3.5 h-3.5 rounded-full border-2 border-white shadow-xs ${isCompleted ? 'bg-emerald-500' : isCancelled ? 'bg-rose-500' : 'bg-slate-300'}`} />
              <p className="text-xs font-black text-slate-900">
                {isCompleted ? 'Service Completed & Verified' : isCancelled ? 'Booking Cancelled' : 'Doorstep Service Completion'}
              </p>
              <p className="text-[11px] text-slate-400 mt-0.5">
                {isCompleted ? 'Warranty active for 7 days' : isCancelled ? 'Booking terminated' : 'Verified with OTP upon finishing'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-5 border border-slate-200/80 shadow-xs flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2 text-xs divide-y divide-slate-100 mt-1">
            <div className="flex justify-between items-center pt-1">
              <span className="text-slate-500 font-medium">Standard Service Charge</span>
              <span className="font-bold text-slate-800">₹{booking.totalPrice ?? 499}</span>
            </div>
            <div className="flex justify-between items-center pt-2">
              <span className="text-slate-500 font-medium">Payment Mode</span>
              <span className="font-bold text-brand-blue uppercase text-[11px]">
                {booking.paymentMode === 'advance' ? 'Advance Paid Online' : 'Pay After Service (Cash / UPI)'}
              </span>
            </div>
            <div className="flex justify-between items-center pt-2 text-sm">
              <span className="font-black text-slate-900">Total Payable</span>
              <span className="font-black text-slate-900">₹{booking.totalPrice ?? 499}</span>
            </div>
          </div>
        </div>
