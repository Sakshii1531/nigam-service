...
const Dashboard = ({ defaultType }) => {
  ...
  function fromCms(segment, fallback) {
    ...
    const rows = cmsBanners.filter((b) =>
      segment === "warranty"
        ? b.segment === "warranty"
        : b.segment !== "warranty",
    );
    return rows.length
      ? rows.map((b) => ({ id: b.id, image: b.imageUrl }))
      : fallback;
  }
  ...
  return (
    <div className="min-h-screen bg-bg-light flex flex-col pb-16 lg:pb-0">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-md p-6 shadow-2xl flex flex-col gap-5">
            ...
            <p className="text-sm text-text-primary">
              Please provide your bill details to claim free service under
              warranty.
            </p>
            ...
            <div className="flex gap-3 mt-2">
              <button
                onClick={() => {
                  if (!billNo && !billFile) {
                    alert(
                      "Please provide Bill No or Upload Bill to claim warranty.",
                    );
                    return;
                  }
            ...
            <div className="mt-4 md:mt-6 pt-3 text-[9.5px] md:text-xs font-bold text-slate-500 border-t border-transparent md:border-slate-100/80">
              <div className="flex flex-col md:flex-row items-center justify-center gap-3 md:gap-8 w-max mx-auto px-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    Click on 'Visit Official Site' to go to brand website
                  </span>
                </div>
              </div>
            </div>
      ...
      <CustomerBottomNav
        buyPath={activeType === "in-warranty" ? "/extend-warranty" : "/buy"}
      />
