...
const Buy = () => {
  ...
  const selectedAppliance = params.appliance
    ? decodeURIComponent(params.appliance)
    : null;
  ...
  useEffect(() => {
    if (step === 8) {
      const fetchPlans = async () => {
        ...
        try {
          const wRes = await apiRequest(
            "/warranty-amc/extended-warranty/orders",
            { auth: true },
          );
          const aRes = await apiRequest("/warranty-amc/amc/subscriptions", {
            auth: true,
          });
  ...
  const selectedTier =
    selectedTiers[selectedTierIndex] || selectedTiers[0] || null;
  ...
  const handleBuyWarranty = async () => {
    ...
    try {
      ...
      navigate(
        `/buy/success/${encodeURIComponent(selectedAppliance || "")}/${selectedTierIndex}`,
      );
    } catch (err) {
      setBuyError(
        err.message ||
          "The policy could not be activated. You have not been charged.",
      );
    } finally {
      setBuying(false);
    }
  ...
  const defaultBrand = applianceBrands[0] || "";
          ...
          <button
            onClick={() => {
              if (step === 2) navigate("/buy");
                ...
                navigate(
                  `/buy/select-tier/${encodeURIComponent(selectedAppliance || "")}`,
                );
              else if (step === 4)
                navigate(
                  `/buy/enter-details/${encodeURIComponent(selectedAppliance || "")}/${selectedTierIndex}`,
                );
              else if (step === 6)
                navigate(
                  `/buy/review/${encodeURIComponent(selectedAppliance || "")}/${selectedTierIndex}`,
                );
                ...
                if (claimPlanType === "Extended Warranty")
                  navigate("/buy/warranty-details");
      ...
      <div className="flex-1 p-3.5 sm:p-6 md:px-8 flex flex-col gap-4 sm:gap-6 md:gap-8 overflow-y-auto max-w-screen-2xl mx-auto w-full">
              ...
              <button
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <ChevronDown className="h-3 w-3 text-text-secondary group-hover:text-brand-blue group-hover:translate-y-0.5 transition-all shrink-0" />
                  </div>
                </div>
              </button>
              ...
              {selectedTiers.map((tier, idx) => {
                ...
                return (
                  <div
                    ...
                    onClick={() =>
                      navigate(
                        `/buy/select-tier/${encodeURIComponent(selectedAppliance || "")}?tier=${idx}`,
                      )
                    }
        ...
        {step === 4 && (
          <motion.div
              ...
              <div className="w-16 h-16 bg-slate-50 border border-slate-100 rounded-xl flex items-center justify-center p-1.5 shrink-0">
                <img
                  src={
                    ...
                    selectedAppliance?.toLowerCase().includes("television")
                      ? tvImg
                      : selectedAppliance?.toLowerCase().includes("fridge") ||
                          selectedAppliance
                            ?.toLowerCase()
                        ...
                        : selectedAppliance
                              ?.toLowerCase()
                              .includes("washing") ||
                            selectedAppliance?.toLowerCase().includes("machine")
                          ? washingImg
                          : selectedAppliance?.toLowerCase().includes("ac") ||
                              selectedAppliance
                                ?.toLowerCase()
            ...
            <div className="bg-white border border-slate-200/80 rounded-2xl p-4 flex flex-col gap-3 shadow-sm">
              ...
              <div className="border-t border-slate-100 pt-3 flex justify-between items-center">
                ...
                <span className="text-xl font-black text-brand-blue">
                  ₹{Number(selectedTier.price || 0).toLocaleString("en-IN")}
                </span>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200/80 rounded-2xl p-4 flex flex-col gap-2.5 shadow-sm">
              <h4 className="text-xs font-black text-brand-navy mb-0.5">
                Benefits Included?
              </h4>
              {[
                "Manufacturing Defects",
        ...
        {step === 6 && (
          <motion.div
            ...
            {/* Recommended Section */}
            <div className="flex flex-col gap-3">
              <span className="text-[11px] font-black text-brand-navy uppercase tracking-wider block px-1">
                Recommended
              </span>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between shadow-sm mt-2">
              <div>
                ...
                <span className="text-lg font-black text-brand-navy block mt-0.5">
                  ₹{Number(selectedTier.price || 0).toLocaleString("en-IN")}
                </span>
              </div>
            ...
            <div className="flex flex-col gap-2.5 mt-2">
              <button
                ...
                {buying
                  ...
                  : `Pay ₹${Number(selectedTier?.price || 0).toLocaleString("en-IN")} Securely`}
              </button>
              {buyError && (
                <p className="text-[11px] font-bold text-red-600 text-center px-2">
                  {buyError}
                </p>
              )}
          ...
          <motion.div
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-text-secondary mt-1.5 font-medium">
                Your appliance is now covered under Nigam Shield.
              </p>
            </div>
            ...
            <div className="w-full bg-linear-to-br from-brand-navy via-[#0C3F85] to-brand-blue rounded-3xl p-6 text-white shadow-2xl relative overflow-hidden border border-white/10">
              ...
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <h4 className="text-base font-black mt-2.5 truncate w-48">
              ...
              <div className="flex flex-col gap-3.5 border-t border-white/10 pt-4 text-xs">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-mono tracking-wider font-semibold">
                    {boughtOrder?.humanId || boughtOrder?.id || "—"}
                  </span>
                </div>
                <div className="flex justify-between items-center">
        ...
        {step === 8 && (
          <motion.div
            ...
            <div className="px-1 -mt-2 text-left">
              ...
              <p className="text-xs text-text-secondary font-semibold">
                Track all your active plans & coverage
              </p>
            </div>
            ...
            {plansLoading ? (
              <div className="animate-pulse space-y-4">
                {[1, 2].map((n) => (
                  <div
                    key={n}
                    className="bg-white border border-slate-200 rounded-3xl p-4.5 flex gap-4 h-36"></div>
                ))}
              </div>
                ...
                {(() => {
                  const filteredWarranties = warranties.filter((w) => {
                    ...
                    if (activeWarrantyTab === "Expiring Soon") {
                      return (
                        w.status === "Active" && daysLeft <= 30 && daysLeft > 0
                      );
                    }
                    ...
                    <div className="flex flex-col gap-4">
                      {filteredWarranties.map((w, idx) => {
                        const img = (() => {
                          ...
                          if (cat.includes("wash") || cat.includes("washing"))
                            return washingImg;
                          return kitchenApplianceImg;
                        })();

                        const orderId = w.humanId || w.id;
                        const validTillStr = new Date(
                          w.validTill,
                          ...
                          <div
                            ...
                            <div className="flex-1 flex flex-col gap-1 min-w-0">
                              <div className="flex items-center justify-between gap-2">
                                <h4 className="text-sm font-black text-brand-navy leading-tight truncate">
                                  {w.category || "Appliance"}
                                </h4>
                              ...
                              <span className="text-[10px] text-brand-blue font-extrabold block">
                                {w.planDurationYears || 1} Year Extended
                                Warranty
                              </span>
                              ...
                              <button
                                onClick={() => {
                                  setSelectedWarranty({
                                    ...
                                    terms:
                                      "Mirrors the manufacturer's original coverage. Does not cover any accidental physical damage, liquid spills, or services performed by unauthorized local workshops.",
                                  });
                                  setStep(10);
                                }}
                      ...
                      return (
                        <div
                          ...
                          <div className="flex-1 flex flex-col gap-1 min-w-0">
                            ...
                            <span className="text-[10px] text-brand-blue font-extrabold block">
                              {a.planName || "Comprehensive AMC Plan"}
                            </span>
                            ...
                            <button
                              onClick={() => {
                                setSelectedAMC({
                                  ...
                                  terms:
                                    "Includes replacement of consumable filters once per contract year. Excludes booster pump motor replacement or physical tank breakage.",
                                });
                                setStep(13);
                              }}
                              className="w-full border border-[#0B4EA2]/30 hover:border-[#0B4EA2]/60 hover:bg-blue-50/20 text-[#0B4EA2] font-black text-xs py-2.5 rounded-xl mt-3 transition-colors cursor-pointer">
        ...
        {step === 9 && (
          <motion.div
            ...
            <div className="bg-linear-to-br from-[#E3F2FD] to-[#BBDEFB] rounded-3xl p-5 border border-blue-100 shadow-sm flex items-center gap-4 relative overflow-hidden">
              ...
              <div className="flex-1">
                ...
                <p className="text-[10px] text-slate-700 leading-relaxed font-semibold mt-0.5">
                  100% cashless, completely paperless protection for your
            ...
            <div className="flex flex-col gap-3">
              <h4 className="text-xs font-black text-brand-navy uppercase tracking-wider px-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  },
                  {
                    ...
                    icon: (
                      <ShieldCheck className="h-4.5 w-4.5 text-brand-navy" />
                    ),
                  },
                ].map((item, idx) => (
                  <div
                    key={idx}
                    className="bg-white border border-slate-200/80 rounded-2xl p-4 flex flex-col gap-2 shadow-xs">
                    <div className="w-8 h-8 rounded-xl bg-slate-50 flex items-center justify-center border border-slate-100 shadow-2xs">
                      {item.icon}
                    </div>
                    <h5 className="text-[11px] font-extrabold text-brand-navy leading-snug">
                      {item.title}
                    </h5>
              ...
              <div className="bg-white border border-slate-200 rounded-3xl p-4 shadow-sm flex flex-col gap-3.5">
                {[
                  ...
                  <div
                    ...
                    <span className="text-[10px] font-extrabold text-[#0B4EA2] uppercase tracking-wider">
                      FAQ {idx + 1}
                    </span>
                    <h5 className="text-xs font-black text-brand-navy mt-0.5">
                      {faq.q}
                    </h5>
                    <p className="text-[10px] text-text-secondary leading-relaxed font-semibold mt-1">
                      {faq.a}
        ...
        {step === 10 && selectedWarranty && (
          <motion.div
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-5 flex gap-4 shadow-sm relative overflow-hidden">
              ...
              <div className="flex-1 flex flex-col gap-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <h3 className="text-base font-black text-brand-navy leading-tight truncate">
                ...
                <span className="text-[11px] text-brand-blue font-extrabold block">
                  {selectedWarranty.plan}
                </span>
              </div>
            </div>
            ...
            <div className="bg-linear-to-br from-brand-navy to-brand-blue rounded-3xl p-6 text-white shadow-md flex flex-col gap-4 relative overflow-hidden border border-blue-900/10">
              ...
              <div className="flex flex-col gap-3 text-xs">
                <div className="flex justify-between">
                  ...
                  <strong className="font-mono tracking-wider text-white font-extrabold">
                ...
                <div className="flex justify-between">
                  ...
                  <strong className="text-brand-yellow font-extrabold">
                    {selectedWarranty.validTill}
                  </strong>
                </div>
              </div>
            </div>
            ...
            <div className="w-full flex flex-col gap-3 mt-2">
              ...
              <button
                onClick={() => {
                  alert(
                    "Your warranty certificate will be emailed once the purchase is confirmed.",
                  );
                }}
        ...
        {step === 11 && (
          <motion.div
            ...
            <div className="flex flex-col gap-2 px-1">
              <h3 className="text-sm font-black text-[#0B4EA2]">
                Shop by Category
              </h3>
                    ...
                    <div className="w-7 h-7 flex items-center justify-center">
                      {item.name === "For You" && (
                        <svg
                          ...
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          className="w-7 h-7 text-[#0B4EA2]">
                          <path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L21 12l-5.813-1.912a2 2 0 0 1-1.275-1.275Z" />
                          <path
                            d="m5 3 1 2.5L8.5 6 6 7 5 9.5 4 7 1.5 6 4 5Z"
                            opacity="0.5"
                          />
                          <path
                ...
                {[
                  ...
                  <div
                    ...
                    {item.isText ? (
                      ...
                      <div className="flex items-center gap-1">
                        <div className="w-4.5 h-4.5 rounded-full bg-[#C6004E] flex items-center justify-center text-white text-[8px] font-bold">
                          L
                        </div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex justify-between items-center px-1">
                ...
                <span className="text-xs font-bold text-[#0B4EA2] hover:underline cursor-pointer">
                  View All
                </span>
              </div>
                  ...
                  return (
                    <div
                      ...
                      <div>
                        ...
                        <span className="text-[8px] text-slate-400 leading-tight block truncate mt-0.5">
                          {item.desc}
                        </span>
            ...
            <div className="bg-slate-50/50 border border-slate-200 rounded-3xl p-4.5 flex justify-between items-center gap-2 mt-0">
              {[
                ...
                <div
                  ...
                  <div>
                    <h5 className="text-[9px] font-extrabold text-brand-navy leading-snug">
                      {badge.title}
                    </h5>
                    <p className="text-[8px] text-slate-500 leading-none mt-0.5 font-bold">
                      {badge.sub}
                    </p>
                  </div>
                </div>
        ...
        {step === 12 && (
          <motion.div
            ...
            <div className="px-1 -mt-2">
              ...
              <p className="text-xs text-text-secondary font-semibold">
                Select an appliance to get extended warranty protection
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {(ewAppliances || []).map((a) => ({
                ...
                <div
                  ...
                  <div>
                    <h4 className="text-xs font-black text-brand-navy leading-tight">
                      {item.name}
                    </h4>
        ...
        {step === 13 && selectedAMC && (
          <motion.div
            ...
            <div className="bg-linear-to-br from-brand-navy to-[#059669] rounded-3xl p-6 text-white shadow-md flex flex-col gap-4 relative overflow-hidden border border-emerald-900/10 text-left">
              ...
              <div className="flex justify-between items-center border-b border-white/10 pb-3">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-[#FFD54F]">
              ...
              <div className="flex flex-col gap-3 text-xs font-semibold">
                ...
                <div className="flex justify-between">
                  <span className="text-white/60">Valid Till:</span>
                  <strong className="text-[#FFD54F] font-extrabold">
                    {selectedAMC.validTill}
                  </strong>
                </div>
              </div>
            </div>
            ...
            <div className="w-full flex flex-col gap-3 mt-2">
              ...
              <button
                onClick={() => {
                  alert(
                    "Your certificate will be emailed once the purchase is confirmed.",
                  );
                }}
                className="w-full bg-slate-100 hover:bg-slate-200 text-brand-navy font-bold py-3 rounded-xl transition-all text-xs cursor-pointer">
        ...
        {step === 14 && (
          <motion.div
            ...
            <div className="px-1 -mt-2">
              ...
              <h2 className="text-lg font-black text-brand-navy mt-1.5">
                Raise claim for {claimSubject}
              </h2>
              <p className="text-xs text-text-secondary font-semibold">
                Policy Order ID: {claimOrderId}
              </p>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-5 flex flex-col gap-5 shadow-sm">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-black text-brand-navy uppercase tracking-wider pl-1 font-bold">
                  Describe the Issue
                </label>
                <textarea
                  value={claimIssue}
            ...
            <div className="w-full flex flex-col gap-3 mt-2">
              <button
                onClick={() => {
                  ...
                  if (!claimDate) {
                    alert(
                      "Please select a preferred date for the appointment.",
                    );
                    return;
                  }

                  const generatedId =
                    "NCC-CLM-" + Math.floor(100000 + Math.random() * 900000);
          ...
          <motion.div
            ...
            <div>
              ...
              <p className="text-xs text-text-secondary font-semibold mt-1">
                Our serviceProvider team has been notified.
              </p>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm w-full flex flex-col gap-3 text-left">
              <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                <span className="text-[10px] font-black text-brand-navy uppercase tracking-wider font-bold">
                  Ticket Summary
                </span>
                <span className="text-xs font-mono font-black text-[#0B4EA2]">
                  {submittedTicketId}
                </span>
              </div>
