    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col pb-10 lg:pb-8">
      ...
      <div className="bg-white px-4 py-4 flex items-center gap-3 sticky top-0 z-50 shadow-sm border-b border-slate-100 lg:hidden">
        ...
        <div className="flex-1">
          ...
          {unread > 0 && <span className="text-[11px] text-brand-blue font-semibold">{unread} unread</span>}
        </div>
        <button
      ...
      <div className="hidden lg:block max-w-screen-2xl mx-auto w-full px-6 xl:px-8 pt-6 pb-2">
        <div className="flex items-center justify-between bg-white rounded-3xl p-5 border border-slate-200/80 shadow-2xs">
          <div className="flex items-center gap-3.5">
            <button
              onClick={() => navigate(-1)}
              className="p-2 bg-slate-100 hover:bg-slate-200 rounded-2xl text-brand-navy transition-colors cursor-pointer"
              title="Back"
            >
              <ArrowLeft className="h-5 w-5 stroke-[2.5]" />
            </button>
            <div>
              <h1 className="text-xl font-black text-brand-navy tracking-tight">Notifications</h1>
          ...
          <div className="flex items-center gap-3">
            {unread > 0 && (
              <button 
                onClick={markAllRead} 
                className="px-4 py-2 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-2xl text-xs font-bold text-brand-blue transition-all cursor-pointer shadow-xs"
              >
        ...
        {items.map((n) => {
          ...
          return (
            <button
              key={n.id}
              onClick={() => open(n)}
              className={`text-left bg-white border rounded-2xl p-4 flex gap-3 items-start shadow-sm transition-colors hover:border-brand-blue/30 ${
                n.read ? 'border-slate-100' : 'border-brand-blue/20 bg-[#FBFCFF]'
              }`}
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  {n.cta && (
                    <span className="flex items-center gap-0.5 text-[11px] font-semibold text-brand-blue">
