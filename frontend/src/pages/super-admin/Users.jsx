...
const Users = () => {
  ...
  const formatUserAddress = (a) => {
    ...
    const city =
      !isEmbedded && a.city && a.city !== "Delhi"
        ? a.city.trim()
        : !isEmbedded
          ? (a.city || "").trim()
          : "";
    const pincode =
      !isEmbedded && a.pincode && a.pincode !== "110001"
        ? a.pincode.trim()
        : !isEmbedded
          ? (a.pincode || "").trim()
          : "";
  ...
  const handleViewProfile = async (user) => {
    ...
    try {
      const fullUser = await apiRequest(
        `/super-admin/users/${user._id || user.id}`,
        { auth: true },
      );
      if (fullUser) {
        const formatted = {
          ...
          addresses: fullUser.addresses
            ? fullUser.addresses.map(formatUserAddress)
            : [],
      ...
      console.warn(
        "Could not load complete customer profile, using list item:",
        err,
      );
    } finally {
    ...
    async function fetchUsers() {
      try {
        const res = await apiRequest(
          "/super-admin/users?role=customer&limit=200",
          { auth: true },
        );
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      ...
      setUsers(previous);
      if (selectedUser?.id === id)
        setSelectedUser(previous.find((u) => u.id === id) || null);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await apiRequest(`/super-admin/users/${user._id}`, {
        method: "DELETE",
        auth: true,
      });
  ...
  const filteredUsers = users.filter((u) => {
    ...
    const matchesSource =
      selectedSource === "All Sources" || u.source === selectedSource;
    const matchesMinAppliances =
      !minAppliances || u.appliances >= Number(minAppliances);
    return (
      matchesSearch && matchesStatus && matchesSource && matchesMinAppliances
    );
  });
          ...
          <div className="p-6 space-y-6 flex-1 bg-[#F8FAFC] text-left">
            <div className="flex items-center justify-between">
              <button
                ...
                <ArrowLeft size={14} className="stroke-[2.5]" /> Back to
                Customers
              </button>
            </div>
                  ...
                  <div className="flex flex-col">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span
                        ...
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${selectedUser.status === "Active" ? "bg-emerald-500 animate-pulse" : "bg-rose-500"}`}
                        />
                        {selectedUser.status}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 mt-1">
                      ...
                      <span>•</span>
                      <span className="bg-slate-100 text-slate-700 px-2 py-0.5 rounded-md font-bold text-[10px] uppercase tracking-wider">
                        {selectedUser.source || "B2C"}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          ...
                          <div className="bg-slate-50/70 border border-slate-200/80 rounded-2xl p-5 space-y-4">
                            <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                              <Info size={14} className="text-[#0D47A1]" />{" "}
                              Contact Information
                            </h4>
                              ...
                              {(!selectedUser.addresses ||
                                ...
                                <div className="bg-white p-6 rounded-xl border border-slate-200/80 text-center">
                                  ...
                                  <p className="text-xs text-slate-500 font-medium">
                                    No saved addresses on file yet.
                                  </p>
                                </div>
                              )}
                            </div>
                    ...
                    {activeTab === "appliances" && (
                      <div className="space-y-6">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {selectedUser.applianceList?.map((app, idx) => (
                              <div
                                key={idx}
                                className="p-3 border border-[#E2E8F0] rounded-xl flex items-center justify-between bg-white shadow-xs">
                                <div className="flex items-center gap-2.5">
                                ...
                                <span className="text-[10px] text-[#0D47A1] font-bold bg-[#EEF4FF] px-2 py-0.5 rounded-full">
                                  Active
                                </span>
                              </div>
                            ))}
                            {(!selectedUser.applianceList ||
                              selectedUser.applianceList.length === 0) && (
                        ...
                        <div className="space-y-4">
                          <h4 className="text-xs font-bold text-[#64748B] uppercase tracking-wider">
                            Purchased Subscriptions & Policies
                          </h4>

                          {/* AMC Plans */}
                          <div className="space-y-2">
                            <span className="text-[10px] font-black text-[#0D47A1] uppercase tracking-widest bg-blue-50 px-2 py-1 rounded-md">
                              AMC Plans (
                              {selectedUser.amcSubscriptions?.length || 0})
                            </span>
                            <div className="grid grid-cols-1 gap-2 mt-1">
                              {selectedUser.amcSubscriptions?.map(
                                (sub, idx) => (
                                  <div
                                    key={idx}
                                    className="p-3.5 border border-[#E2E8F0] rounded-xl bg-white flex flex-wrap justify-between items-center gap-3">
                                    <div>
                                      <p className="text-xs font-bold text-[#1E293B]">
                                        {sub.planName ||
                                          sub.plan?.name ||
                                      ...
                                      <p className="text-[10px] text-[#64748B] font-semibold mt-0.5">
                                        Contract: {sub.contractNo} | Appliance:{" "}
                          ...
                          {/* Extended Warranties */}
                          <div className="space-y-2">
                            <span className="text-[10px] font-black text-indigo-700 uppercase tracking-widest bg-indigo-50 px-2 py-1 rounded-md">
                              Extended Warranties (
                              {selectedUser.warrantyOrders?.length || 0})
                            </span>
                            <div className="grid grid-cols-1 gap-2 mt-1">
                              {selectedUser.warrantyOrders?.map(
                                (policy, idx) => (
                                  <div
                                    key={idx}
                                    className="p-3.5 border border-slate-100 rounded-xl bg-white flex flex-wrap justify-between items-center gap-3">
                                    <div>
                                      <p className="text-xs font-bold text-[#1E293B]">
                                        {policy.brand}{" "}
                                        {policy.applianceCategory ||
                                          "Appliance"}{" "}
                                        Warranty
                                      </p>
                                      <p className="text-[10px] text-[#64748B] font-semibold mt-0.5">
                                        Policy: {policy.policyId} | Model:{" "}
                                        {policy.modelNumber || "—"}
                                      </p>
                                      <p className="text-[9px] text-[#94A3B8] font-bold mt-0.5">
                                        Valid Till:{" "}
                              ...
                              {(!selectedUser.warrantyOrders ||
                                selectedUser.warrantyOrders.length === 0) && (
                                <p className="text-xs text-slate-500 italic pl-1">
                                  No active extended warranty policies.
                                </p>
                              )}
                            </div>
                          </div>
                    ...
                    {activeTab === "services" && (
                      <div className="space-y-4">
                        <h4 className="text-xs font-bold text-[#64748B] uppercase tracking-wider">
                          ...
                          {selectedUser.serviceRequests?.length || 0})
                        </h4>
                        <div className="space-y-3">
                          {selectedUser.serviceRequests?.map((req) => {
                            ...
                            return (
                              <div
                                key={req._id}
                                className="p-4 border border-[#E2E8F0] rounded-xl bg-white shadow-xs space-y-3">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs border-t border-slate-100 pt-2 font-semibold">
                                  <div>
                                    ...
                                    {req.appliance?.brand || req.brand}{" "}
                                    {req.appliance?.type || req.category}
                                  </div>
                                  <div>
                                    <span className="text-[10px] text-slate-400 block font-normal">
                                      COMPLAINT TYPE
                    ...
                    {activeTab === "orders" && (
                      <div className="space-y-6">
                              ...
                              return (
                                <div
                                  ...
                                  <div className="border-t border-slate-100 pt-2 text-xs font-semibold">
                                    <span className="text-[10px] text-slate-400 block font-normal mb-1.5">
                                      ITEMS
                                    </span>
                        ...
                        <div className="space-y-3">
                          <h4 className="text-xs font-bold text-[#64748B] uppercase tracking-wider">
                            Device Exchange Valuations (
                            {selectedUser.exchangeRequests?.length || 0})
                          </h4>
                          <div className="space-y-3">
                            ...
                            {(!selectedUser.exchangeRequests ||
                              selectedUser.exchangeRequests.length === 0) && (
                              <p className="text-xs text-slate-500 italic pl-1">
                                No exchange requests submitted.
                              </p>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="space-y-5">
                        ...
                        <div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-lg font-black text-slate-800 tracking-wider mt-0.5">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2">
                            ...
                            {(!selectedUser.referrals ||
                              selectedUser.referrals.length === 0) && (
                              <p className="text-xs text-slate-500 italic pl-1">
                                No signups registered with this referral code
                                yet.
                              </p>
                            )}
                          </div>
              ...
              <div className="p-4 border-t border-slate-100 bg-slate-50/80 flex flex-wrap items-center justify-between gap-4">
                ...
                {selectedUser.status === "Active" ? (
                  ...
                  <button
                    ...
                    <CheckCircle size={15} className="stroke-[2.5]" /> Activate
                    Account
                  </button>
                )}
              </div>
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-500 mt-0.5">
                    View and manage registered accounts, services, warranties,
                    and status controls.
                  </p>
                </div>
                <div className="flex gap-2">
                  <span className="text-xs bg-white border border-slate-200 px-3 py-1.5 rounded-full text-slate-600 font-bold shadow-2xs">
                    Total Users:{" "}
                    <strong className="text-[#0D47A1]">{users.length}</strong>
                  </span>
                </div>
              </div>
                ...
                <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-2xs flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-2xl font-black text-purple-600 mt-0.5 block">
                      {
                        users.filter((u) => u.source && u.source !== "B2C")
                          .length
                      }
                    </span>
                  </div>
              ...
              <div className="flex flex-wrap gap-4 items-center justify-between">
                ...
                <button
                  ...
                  className={`border px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 ${
                    showExtraFilters
                      ? "bg-[#EEF4FF] border-[#0D47A1] text-[#0D47A1]"
            ...
            <div className="bg-white rounded-2xl border border-[#E2E8F0] overflow-hidden shadow-sm">
              ...
              {sortedUsers.length === 0 && (
                <div className="text-center py-12 bg-white">
                  <UsersIcon
                    size={48}
                    className="text-[#64748B] mx-auto mb-4 opacity-50 text-slate-400"
                  />
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-2xl max-w-sm w-full p-6 space-y-4 text-center animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500 font-semibold leading-relaxed">
                ...
                account? This action cannot be undone and will remove the
