    ...
    for (const [key, placement] of Object.entries(TILE_PLACEMENTS)) {
      ...
      for (const staleId of staleIds) {
        if (!String(staleId).match(/^\d+$/)) {
          apiRequest(`/cms/home-tiles/${staleId}`, {
            method: "DELETE",
            auth: true,
          }).catch(() => {});
        }
...
async function writeTiles(key, list) {
  ...
  try {
    for (const [i, tile] of list.entries()) {
      ...
      const directId =
        tile.id && !String(tile.id).match(/^\d+$/) ? String(tile.id) : null;
      const cacheMatch = previous.find(
        (p) =>
          (p.id && tile.id && String(p.id) === String(tile.id)) ||
          (p.name &&
            tile.name &&
            p.name.trim().toLowerCase() === tile.name.trim().toLowerCase()) ||
          (p.title &&
            tile.title &&
            p.title.trim().toLowerCase() === tile.title.trim().toLowerCase()),
      );
      const targetId =
        directId ||
        (cacheMatch?.id && !String(cacheMatch.id).match(/^\d+$/)
          ? String(cacheMatch.id)
          : null);

      if (targetId) {
        const updated = await apiRequest(`/cms/home-tiles/${targetId}`, {
          method: "PUT",
          auth: true,
          body,
        });
        ...
        const created = await apiRequest("/cms/home-tiles", {
          method: "POST",
          auth: true,
          body,
        });
      ...
      (prev) =>
        ...
        !list.some(
          (t) =>
            ...
            (t.title &&
              prev.title &&
              t.title.trim().toLowerCase() === prev.title.trim().toLowerCase()),
        ),
    );

    for (const gone of goneTiles) {
      await apiRequest(`/cms/home-tiles/${gone.id}`, {
        method: "DELETE",
        auth: true,
      });
    }
...
const CustomerAppCustomization = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const all = await apiRequest("/cms/banners/admin?app=customer", {
          auth: true,
        });
        const list = Array.isArray(all) ? all : [];
        setNonWarrantyBanners(
          list.filter((b) => b.segment !== "warranty").map(shapeBanner),
        );
        setWarrantyBanners(
          list.filter((b) => b.segment === "warranty").map(shapeBanner),
        );
  ...
  const detectIconFromName = (name) => {
    ...
    if (
      ...
      return "gardening";
    if (
      norm.includes("carpent") ||
      norm.includes("wood") ||
      norm.includes("furniture repair")
    )
      return "carpenter";
    if (norm.includes("appliance") || norm.includes("plug")) return "appliance";
    if (
      norm.includes("ac") ||
      norm.includes("air condition") ||
      norm.includes("cool")
    )
      return "ac";
    if (
    ...
    if (
      ...
      return "tv";
    if (norm.includes("ro") || norm.includes("purif") || norm.includes("water"))
      return "ro";
    if (
      norm.includes("geyser") ||
      norm.includes("heater") ||
      norm.includes("boiler")
    )
      return "geyser";
  ...
  const handleResetCategories = () => {
    if (
      window.confirm(
        "Reset categories to default customer app dashboard options?",
      )
    ) {
      setCategories(DEFAULT_CATEGORIES);
      writeTiles("categories", DEFAULT_CATEGORIES);
  ...
  const handleAddBanner = async (e) => {
    ...
    try {
      ...
      if (bannerType === "non-warranty")
        setNonWarrantyBanners((prev) => [...prev, banner]);
  ...
  const handleDeleteBanner = async (id) => {
    ...
    if (bannerType === "non-warranty")
      setNonWarrantyBanners((p) => p.filter((b) => b.id !== id));
  ...
  const handleResetBanners = () => {
    showToast(
      "Banners are live content — delete individual banners instead of resetting.",
    );
  };

  const currentBanners =
    bannerType === "non-warranty" ? nonWarrantyBanners : warrantyBanners;
  ...
  const handleDeleteStory = async (index) => {
    const story = storiesList[index];
    if (
      !window.confirm(`Are you sure you want to delete story "${story.title}"?`)
    )
      return;
    ...
    try {
      await apiRequest(`/cms/stories/${story.id}`, {
        method: "DELETE",
        auth: true,
      });
  ...
  const handleSaveStory = async (e) => {
    ...
    try {
      if (isEditingStory) {
        ...
        const saved = await apiRequest(`/cms/stories/${existing.id}`, {
          method: "PUT",
          auth: true,
          body,
        });
        setStoriesList(
          storiesList.map((s2, i) =>
            i === editStoryIndex ? shapeStory(saved) : s2,
          ),
        );
      } else {
        const saved = await apiRequest("/cms/stories", {
          method: "POST",
          auth: true,
          body,
        });
              ...
              <div className="flex flex-wrap justify-between items-center bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-500 font-medium pl-10">
                    Configure the top category bar icons, booking services, and
                    target routes shown on the customer app home screen.
                  </p>
                </div>
                          ...
                          <td className="px-6 py-4">
                            {cat.icon && cat.icon.startsWith("data:image/") ? (
                              <img
                                src={cat.icon}
                                alt={cat.name}
                                className="w-9 h-9 object-contain border border-slate-200 rounded-xl p-1 bg-slate-50 shadow-2xs"
                              />
                            ) : (
                              <div className="w-10 h-10 rounded-2xl bg-blue-50/80 border border-blue-100/90 flex items-center justify-center shadow-2xs group-hover:scale-105 transition-transform">
                                <CategoryVectorIcon
                                  iconKey={
                                    cat.icon || detectIconFromName(cat.name)
                                  }
                                  className="w-5.5 h-5.5 text-[#0D47A1]"
                                />
                              </div>
                            )}
                          </td>
                              ...
                              {cat.isForYou
                                ...
                                : cat.isMore
                                  ...
                                  : cat.isFridge
                                    ...
                                    : cat.service
                                      ? `🛠️ Service: ${cat.service}`
                                      : `📅 Booking: /book/${cat.name}`}
                            </span>
                          </td>
              ...
              <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-[#E2E8F0] shadow-xs">
                <div>
                  <h2 className="text-sm font-bold text-[#1E293B]">
                    Brands & Offers Customization
                  </h2>
                  <p className="text-xs text-slate-500 mt-1">
                    Manage the brand promotion cards and offers shown in the
                    carousel on the user dashboard.
                  </p>
                </div>
                        ...
                        <td className="px-6 py-4">
                          {bc.image && (
                            <img
                              src={bc.image}
                              alt={bc.brandName}
                              className="w-10 h-10 object-contain border border-slate-200 rounded-lg p-0.5 bg-slate-50"
                            />
                          )}
                        </td>
                        <td className="px-6 py-4 text-right">
            ...
            <div className="space-y-6 animate-in fade-in duration-200">
              <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-[#E2E8F0] shadow-xs">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Manage the Instagram-style story cards and sub-slides shown
          ...
          {activeSubSection === "banners" && (
            <div className="space-y-6 animate-in fade-in duration-200">
              <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-[#E2E8F0] shadow-xs">
                <div>
                  <h2 className="text-sm font-bold text-[#1E293B]">
                    Dashboard sliding Banners
                  </h2>
                  <p className="text-xs text-slate-500 mt-1">
                    Manage the sliding offer banners displayed right under the
                    category bar on the user dashboard.
                  </p>
                </div>
                <button
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                ...
                <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-xs flex flex-col gap-4">
                  ...
                  <form onSubmit={handleAddBanner} className="space-y-4">
                    ...
                    {newBannerFile && (
                      <div>
                        <span className="text-[10px] font-semibold text-[#64748B] mb-1 block">
                          Selected Preview:
                        </span>
                        <img
                          src={newBannerFile}
                ...
                <div className="lg:col-span-2 space-y-4">
                  ...
                  {currentBanners.length === 0 ? (
                    <div className="bg-white border border-[#E2E8F0] rounded-2xl p-8 text-center text-slate-400 font-medium text-xs">
                      No banners uploaded for this category tab yet. Upload one
                      on the left panel!
                    </div>
                  ) : (
            ...
            <div className="flex justify-between items-center shrink-0">
              <div>
                ...
                <p className="text-xs font-medium text-slate-500 mt-0.5">
                  Configure category icon, brands, and appliance types.
                </p>
              </div>
              <button
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setCategoryForm((prev) => ({
                          ...
                          name: newName,
                          icon:
                            !isEditing && detected && iconMode === "preset"
                              ? detected
                              : prev.icon || "ac",
                        }));
                      }}
                            ...
                            <div>
                              <span className="text-xs font-bold text-slate-800 block">
                                {AVAILABLE_ICONS.find(
                                  (i) => i.id === categoryForm.icon,
                                )?.label || categoryForm.icon}
                              </span>
                            ...
                            {AVAILABLE_ICONS.map((ico) => {
                              ...
                              return (
                                <button
                                  ...
                                  <CategoryVectorIcon
                                    iconKey={ico.id}
                                    className={`w-6 h-6 ${isSelected ? "text-[#0D47A1]" : "text-slate-500"}`}
                                  />
                                </button>
                              );
                            })}
                        ...
                        {categoryForm.icon &&
                          categoryForm.icon.startsWith("data:image/") && (
                            <img
                              ...
                              alt="Preview"
                              className="w-9 h-9 object-contain border border-slate-200 rounded-xl p-0.5 bg-white shrink-0 shadow-2xs"
                            />
                          )}
                      </div>
                    )}
        ...
        <div className="fixed inset-0 bg-[#052355]/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg p-6 shadow-2xl border border-slate-100 flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200 max-h-[90vh]">
            <div className="flex justify-between items-center shrink-0">
              <h3 className="text-base font-bold text-[#1E293B]">
                {isEditingBrand
                  ? "Edit Brand Offer Card"
                  : "Add New Brand Offer Card"}
              </h3>
              <button
              ...
              <div className="space-y-4 overflow-y-auto pr-1 flex-1 min-h-0 max-h-[75vh] no-scrollbar">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setBrandForm({ ...brandForm, subtitle: e.target.value })
                    }
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      <option value="from-[#E3F2FD] via-[#F4F9FF] to-[#D5E6FF]">
                        Blue (Lloyd Style)
                      </option>
                      <option value="from-[#E8F5E9] via-[#F6FAF6] to-[#D2E8D4]">
                        Green (Samsung Style)
                      </option>
                      <option value="from-[#F0F4FF] via-[#F7F9FF] to-[#E1E8FF]">
                        Indigo (Daikin Style)
                      </option>
                      <option value="from-[#FCE4EC] via-[#FFF1F3] to-[#F8BBD0]">
                        Pink/Red (LG Style)
                      </option>
                    </select>
                  </div>
      ...
      {showStoryModal && (
        <div className="fixed inset-0 bg-[#052355]/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg p-6 shadow-2xl border border-slate-100 flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200 max-h-[90vh]">
            <div className="flex justify-between items-center shrink-0">
              <h3 className="text-base font-bold text-[#1E293B]">
                {isEditingStory ? "Edit Story Card" : "Add Story Card"}
              </h3>
              <button
                        ...
                        <div
                          ...
                          <div className="flex justify-between items-center border-b border-slate-100 pb-1.5">
                            ...
                            <button
                              ...
                              onClick={() =>
                                setStorySlides(
                                  storySlides.filter((s) => s.id !== slide.id),
                                )
                              }
                            ...
                            <div className="flex items-center gap-2 bg-slate-50 border border-dashed border-slate-200 rounded-md p-1">
                              ...
                              {slide.image && (
                                <img
                                  src={slide.image}
                                  alt=""
                                  className="w-10 h-8 object-contain border border-slate-200 rounded bg-white shrink-0"
                                />
                              )}
                            </div>
                          </div>
