...
5. **Pricing engine**: `RateCard` (brand-admin `CallRatesCharges.jsx`) drives invoice/labor pricing; GST is consistently 18% on services (10% appears once, in Buy.jsx extended-warranty checkout — confirm with product before hardcoding); job types AMC/BrandWarranty/ExtendedWarranty always show parts/labor as ₹0 "Covered" with only extras chargeable.
...
- Is `ServiceRequest`/`Booking`/warranty "ticket" meant to be **one unified table** with a `type` discriminator (recommended, given the technician `Job` view already unifies them), or genuinely separate systems that later merge for the technician?
- GST rate: 18% appears almost everywhere, but Buy.jsx extended-warranty checkout computes 10% — confirm the correct rate(s) per product line.
- Real payment gateway choice (Razorpay key field appears in super-admin Settings.jsx — likely Razorpay).
- Real-time transport choice for chat/tracking (WebSocket vs SSE vs polling) and infra (self-hosted vs managed, e.g. Pusher/Ably/Socket.IO).
- File storage (S3-compatible?) for invoices, photos, signatures, documents.
- Whether brand-admin "sub-roles" and super-admin "roles" should be one unified RBAC system or intentionally separate (brand-scoped vs platform-scoped).
